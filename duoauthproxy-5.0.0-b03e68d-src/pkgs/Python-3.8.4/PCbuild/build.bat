...
rem XXX DUO MOD: Do not try to download any dependencies from the internet. During our build process we will
rem copy python_external_deps to the external folder that this command would fill if we allowed it to run.
rem The reason we need to manually comment this out instead of using -e or setting IncludeInternals to False
rem is because this script takes an all or nothing approach to the externals. If you disable downloading for
rem external dependencies it also won't build them even though we provide them ourselves. Given this we decided to
rem just make a Duo mod.
