...
class BaseLDAPServer(protocol.Protocol):
    ...
    # DUO EDIT D46275: Added response controls.
    def queue(self, id, op, response_controls=None):
        if not self.connected:
            raise LDAPServerConnectionLostException()
        # msg = pureldap.LDAPMessage(op, id=id)
        msg = pureldap.LDAPMessage(op, id=id, controls=response_controls)
    # END EDIT
        if self.debug:
    ...
    def _cbLDAPError(self, reason, name):
        ...
        # DUO EDIT D46276: Added support for debug logging messages
        # This adds built-in logging of all errors regardless of whether
        # the handler chooses to log the error
        if self.debug:
            try:
                log.err(reason)
            except Exception:
                # LDAPException.__str__ may fail.
                log.err()
        # END EDIT
    ...
    def _cbHandle(self, response, id, response_controls=None):
        if response is not None:
            # self.queue(id, response)
            self.queue(id, response, response_controls=response_controls)
    # END EDIT
    ...
    def handle(self, msg):
        ...
        if msg.id == 0:
           ...
           #                         lambda response: self._cbHandle(response, msg.id))
                                     lambda response, response_controls=None: self._cbHandle(
                                        response, msg.id, response_controls=response_controls))
            # END EDIT
