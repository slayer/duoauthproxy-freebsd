...
from duoauthproxy.lib import const, ldap, log
...
class LdapSyncDrpcPlugin(ldap_base.BaseLdapDrpcPlugin):
    ...
    def __init__(self, config):
        ...
        self.service_account_password = config.get_protected_str(
            "{0}_protected".format(ldap_base.CONFIG_BIND_PASSWORD),
            ldap_base.CONFIG_BIND_PASSWORD,
            "",
        )
    ...
    def do_ldap_search(
        ...
        * max_result_size: If paging, stop requesting results after
        ...
        if max_result_size is not None:
            max_result_size = parse_positive_int(max_result_size, "max_result_size")

        if not (filter_text is None or isinstance(filter_text, six.string_types)):
        ...
        try:
            try:
                ...
                if timeout_dc.active():
                    ...
                    raise drpc.CallError(
                        ldap_base.ERR_LDAP_TIMEOUT, {"during": "bind",}
                    )
            ...
            try:
                ...
                if timeout_dc.active():
                    ...
                    log.failure(ldap_base.ERR_LDAP_TIMEOUT)
                    raise drpc.CallError(
                        ldap_base.ERR_LDAP_TIMEOUT, {"during": "search",}
                    )
        finally:
            if timeout_dc.active():
                timeout_dc.cancel()
        ...
        * attributes: Set of lower-case byte-strings.
        ...
        def handle_msg(value, controls, d):
            try:
                if isinstance(value, pureldap.LDAPSearchResultDone):
                    ...
                    if isinstance(
                        e, (ldaperrors.Success, ldaperrors.LDAPSizeLimitExceeded)
                    ):
                    ...
                    obj = {"distinguishedname": [escape_bytes(value.objectName)]}
        ...
        while True:
            if pagination == PAGINATION_TYPE_DISABLE:
                ...
                controls = [
                    (
                        ...
                        criticality,
                        pureber.BERSequence([pureber.BERInteger(page_size), cookie,]),
                    ),
                ]
            ...
            if e is not None:
                ...
                if isinstance(
                    e, ldaperrors.LDAPOperationsError
    ...
    def try_ranged_search(self, client, base_dn, objs):
        ...
        for obj in objs:
            ...
            while True:
                ...
                attr_obj = pureldap.LDAPAttributeDescription("distinguishedname")
                value_obj = pureldap.LDAPAssertionValue(
                    distinguishedname.unescape(obj["distinguishedname"][0])
                )
                filter_object = pureldap.LDAPFilter_equalityMatch(attr_obj, value_obj)

                attributes = [
                    "member;range={0}-{1}".format(member_range[0], member_range[1])
                ]
                try:
...
def next_range(
    previous_range, previous_result_empty, range_len=LDAP_ATTR_DEFAULT_RANGE_LEN
):
...
def escape_bytes(s):
    ...
    return "".join(
        [chr(c) if chr(c) in PRINTABLE else "\\{0:02x}".format(c) for c in s]
    )
