...
from duoauthproxy.lib.radius.challenge import (
...
SCRIPT_INJECT = {
    ...
    JS_TYPE_SONICWALLSRA: (
        '<script>Duo_Init("%(proxy_txid)s", "%(api_host)s");' "</script>"
    ),
}
...
class DuoIFrameRadiusServer(ChallengeResponseRadiusServer, ApiErrorHandling):
    def __init__(self, type, script_uri, script_inject, failmode, duo_client, **kwargs):
    ...
    def _find_challenge(self, request: _ProxyRequest) -> Optional[_ChallengeState]:
        ...
        if self.type != JS_TYPE_F5:
            ...
            return ChallengeResponseRadiusServer._find_challenge(self, request)
    ...
    def get_initial_response(self, request: _ProxyRequest):
        ...
        try:
            ...
            response_packet = self.response_for_api_error(
                request, primary_res, e, primary_res.radius_attrs,
            )
        ...
        challenge_packet = self.create_challenge(
            request, challenge_msg, state=state, challenge_id=challenge_id
        )
    ...
    def get_challenge_response(self, request: _ProxyRequest, state: Dict[str, Any]):
        ...
        try:
            ...
            response_packet = self.response_for_api_error(
                request, state["primary_res"], e, state["primary_res"].radius_attrs,
            )
            defer.returnValue(response_packet)

        self.log_request(request, "Authcookie validation result: %r" % finish_res)
        if finish_res["valid_cookie"] and (finish_res["user"] == request.username):
            success = True
        ...
        if success:
            ...
            defer.returnValue(
                self.create_accept_packet(request, radius_attrs=radius_attrs,)
            )
        else:
            log.auth_standard(
