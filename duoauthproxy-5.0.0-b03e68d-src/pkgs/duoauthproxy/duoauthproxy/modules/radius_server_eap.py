...
from duoauthproxy.lib import duo_async, eap, log, mppe, util
from duoauthproxy.lib.base import AuthResult, ServerModule
...
from duoauthproxy.lib.radius.server import (
    _ProxyRequest,
...
def _format_short(preauth_res: Dict[str, Any]) -> bytes:
    factors = preauth_res["factors"]
    if factors:
        sorted_factors = (
            "'%s'" % factors[k].rstrip("1")
...
class DuoEAPRadiusServer(DuoSimpleRadiusServer, ChallengeResponseRadiusServer):
    ...
    def _create_eap_session(self, request):
        ...
        self.log_request(
            request, "Creating EAP session for user %s" % (request.username)
        )
        return session
    ...
    def get_initial_response(self, request: _ProxyRequest):
        ...
        response = yield self.get_challenge_response(request, {}, session=session)
        defer.returnValue(response)

    @defer.inlineCallbacks
    def get_challenge_response(
        self, request: _ProxyRequest, state: Dict[str, Any], session=None
    ):
        """
        Overridden from abstract method in ChallengeResponseRadiusServer.
    ...
    def accept_session(self, session: EAPSession) -> packet.Packet:
        ...
        recv_key, send_key = mppe.get_mppe_keys(
    ...
    def auto_auth(
        ...
        password: str,
        factor: str = None,
        primary_res: AuthResult = None,
    ):
        """
        Executes primary and secondary authentication.

        Returns:
            packet.Packet: The response to send to the appliance
        """
        if not factor:
            if util.should_try_splitting_password(
                ...
                if not factor:
                    factor = None
                    self.log_request(
                        request, "attempting authentication with factor auto"
                    )
                else:
                    self.log_request(
                        request, "attempting authentication with factor %s" % (factor)
                    )
