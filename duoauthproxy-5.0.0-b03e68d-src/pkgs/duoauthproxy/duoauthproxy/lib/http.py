...
class _ConnectProxyClientProtocol(LineReceiver):
    ...
    def lineReceived(self, line):
        if not self._firstline:
            ...
            fields = line.split()
            try:
                if fields[0] not in (b"HTTP/1.0", b"HTTP/1.1") or fields[1] != b"200":
                    raise ProxyError()
            except (IndexError, ProxyError):
                self.factory.deferred.errback(ProxyError("Proxy Error: %s" % line))
...
class HTTPClient(object):
    ...
    def set_proxy(self, http_proxy_host, http_proxy_port):
        if (http_proxy_host is None) != (http_proxy_port is None):
            raise ValueError(
                "Both http_proxy_host and http_proxy_port must be "
    ...
    def _build_http_factory(
        self, method, url, body, headers, timeout_params, protocol_class=None
    ):
        ...
        if req.port:
            netloc += ":%d" % req.port
        factory_url = urllib.parse.urlunsplit(
            (req.scheme, netloc, req.path or "/", req.query, req.fragment)
        ).encode("utf-8")
    ...
    def request(
        ...
        http_factory = self._build_http_factory(
            method, url, body, headers, timeout_params, protocol_class=protocol_class,
        )
