...
def sign_and_seal_negotiated(flags: int):
    """ Take in a flags number defined by https://docs.microsoft.com/en-us/openspecs/windows_protocols/ms-nlmp/99d90ff4-957f-4c8a-80e4-5bfe5a9a9832
    and return whether sign and seal is being negotiated. """
    seal_negotiated = (flags & NTLMSSP_NEGOTIATE_SEAL) == NTLMSSP_NEGOTIATE_SEAL
    sign_negotiated = (flags & NTLMSSP_NEGOTIATE_SIGN) == NTLMSSP_NEGOTIATE_SIGN

    return seal_negotiated or sign_negotiated


class FieldDescriptor(Packet):
    __byte_order__ = "<"
    __hdr__ = (("Len", "H", 0), ("MaxLen", "H", 0), ("BufferOffset", "L", 0))
...
class NTLMSSPMessageSignature_Extended:
    __byte_order__ = "<"
    __hdr__ = (("Version", "L", 1), ("Checksum", "8s", b"\0" * 8), ("SeqNum", "L", 0))


def create_negotiate_msg(domain="", workstation="", sign_seal=False):
    flags = (
    ...
    if sign_seal:
        flags |= NTLMSSP_NEGOTIATE_SIGN | NTLMSSP_NEGOTIATE_SEAL | NTLMSSP_NEGOTIATE_128
...
def ComputeResponse_v1(
    ...
    if (
        ...
        challenge_digest = hashlib.md5(
            ServerChallenge + ClientChallenge, used_for_security=Depends
        ).digest()
...
def NTOWFv2(password, user, domain):
    ...
    return hmac.new(
        ...
        functools.partial(hashlib.md5, used_for_security=Depends),
    ).digest()


def _create_ntlm_channel_binding_token(appdata):
    ...
    struct_plus_data = struct.pack(
        "<LLLLL{}s".format(application_data_length),
        0,
        0,
        0,
        0,
    ...
    return struct_plus_data


def ComputeResponse_v2(
    NegotiateFlags,
    ResponseKeyNT,
    ResponseKeyLM,
    ServerChallenge,
    ...
    challenge_struct = NTLMv2ClientChallenge(
        TimeStamp=Time, ChallengeFromClient=ClientChallenge, AVPairs=TargetInfo
    )

    if peercert:
        # If there's a peercert, add the channel binding token.
        # Relevant docs:
        #
        ...
        av_pair = AVPair(AvId=AVPair.AVID_MsvChannelBindings)
        appdata = util.create_appdata_from_peercert(peercert)
        cbt_buffer = _create_ntlm_channel_binding_token(appdata)
        av_pair.Value = hashlib.md5(cbt_buffer, used_for_security=Depends).digest()

        # Insert the channel binding AVPair to the front of the list. The list
        # must end with an AvEOL AVPair, which signals the end of the AVPair
        # list, so appending the channel binding token would place it after the
        # AvEOL AVPair and cause it to be ignored
        challenge_struct.AVPairs.insert(0, av_pair)
...
class _NTLMv2_ARC4:
    ...
    def _ksa(self):
        ...
        for i in ARC4_IV:
            ...
            self._keystream[i], self._keystream[j] = (
                self._keystream[j],
                self._keystream[i],
            )
...
class NTLMAuth(object):
    ...
    def _get_auth_msg(self):
        ...
        msg.NegotiateFlags = self.challenge.NegotiateFlags
...
class NTLMv1Auth(NTLMAuth):
    def _compute_response(self, username, password):
        ...
        time = self._get_time()
        return ComputeResponse_v1(
            self.challenge.NegotiateFlags,
            response_key_NT,
            response_key_LM,
...
class NTLMv2Auth(NTLMAuth):
    def __init__(
        ...
        super(NTLMv2Auth, self).__init__(
            challenge_buffer,
            username,
            password,
            domain,
    ...
    def _compute_response(self, username, password):
        response_key_LM = response_key_NT = NTOWFv2(
            password, username, self.domain.upper()
        )
        time = self._get_time()
        return ComputeResponse_v2(
            self.challenge.NegotiateFlags,
            response_key_NT,
            response_key_LM,
            self.challenge.ServerChallenge,
    ...
    def _build_session_key(self):
        if not self._is_key_negotiation():
            session_key = KXKEY_v2(
                self.response[2], self.response[0], self.challenge.ServerChallenge
            )
    ...
    def _build_encrypted_key(self):
        key_exchange_key = KXKEY_v2(
            self.response[2], self.response[0], self.challenge.ServerChallenge
        )

        cipher = _NTLMv2_ARC4.new(key_exchange_key)
        return cipher.update(self.session_key)

    def _build_MIC(self, msg):
        return hmac.new(
            self.session_key,
            self.negotiate_buffer + self.challenge_buffer + bytes(msg),
            functools.partial(hashlib.md5, used_for_security=Depends),
        ).digest()
    ...
    def _get_auth_msg(self):
        ...
        if self._is_key_negotiation():
            msg.EncryptedRandomSessionKey = self._build_encrypted_key()

        msg.MIC = self._build_MIC(msg)

        return msg


def create_ntlm_auth(
    negotiate_buffer: bytes,
    challenge_buffer: bytes,
    username: str,
    password: str,
    domain="",
    workstation="",
    ...
    sign_seal: bool = False,
    peercert: X509 = None,
) -> Union[NTLMAuth, NTLMv2Auth]:

    """ Creates an authenticate object for the specified version of NTLM
    ...
    Args:
        ...
        sign_seal: (bool) True if sign and seal is required
    Returns:
        An NTLM authenticate object contain information based on different version of NTLM

    """
    if ntlm_version == 1:
        auth: NTLMAuth = NTLMv1Auth(
            challenge_buffer, username, password, domain, workstation, client_challenge
        )
    elif ntlm_version == 2:
        auth = NTLMv2Auth(
            negotiate_buffer,
