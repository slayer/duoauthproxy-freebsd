...
class ConfigResultType(Enum):
    ...
    InvalidClientServerMapping = (
        10,
        "The client name '{client_name}' could "
        "not be mapped to a valid client "
        "configuration for the [{section}] section.",
    )
    PortCollision = (
        11,
        "The following server sections have a port collision: {sections}. "
    ...
    IneffectiveConfig = (
        13,
        "{key} in section [{section}] has no effect when {condition}",
    )
    SameSectionPortCollision = (
        14,
        "The following server section has a port collision: {section}. "
        "Port {port} is configured for both SSL and non-SSL connections. "
        "Please modify the ports in your configuration to resolve this.",
    )
    ProtectUnavailable = (
        15,
        "Usage of protected value(s): {keys} is not possible because this machine does not "
        "support our protection library. Please make sure you are running a Windows OS "
        "with access to the Data Protection API.",
    )
    InvalidProtectedValue = (
        16,
        "The protected value for {key} could not be decrypted and therefore cannot be "
    ...
    DeprecatedKey = (
        17,
        "'{key}' has been deprecated and no longer has any effect when used in your configuration.",
    )
    InsecureConfigItem = (
        18,
        "'{key}' in this section is not optimized for security, because {condition}",
    )
    IncompatibleValues = (19, "'{key}' must have type '{type}' when '{condition}'")
...
class ConfigResult(BaseResult):
    ...
    def to_log_output(self, logger):
        logger.emit(
            self.level.twisted_logging_level,
...
SkippedTest = result_creator(
    ConfigResultType.SkippedTest, level=ConfigResultLevel.Warning
)
...
NetworkContention = result_creator(
    ConfigResultType.NetworkContention, level=ConfigResultLevel.Warning
)
...
IneffectiveConfig = result_creator(
    ConfigResultType.IneffectiveConfig, level=ConfigResultLevel.Warning
)
...
DeprecatedKey = result_creator(
    ConfigResultType.DeprecatedKey, level=ConfigResultLevel.Warning
)
InsecureConfigItem = result_creator(
    ConfigResultType.InsecureConfigItem, level=ConfigResultLevel.Warning
)
IncompatibleValues = result_creator(ConfigResultType.IncompatibleValues)
