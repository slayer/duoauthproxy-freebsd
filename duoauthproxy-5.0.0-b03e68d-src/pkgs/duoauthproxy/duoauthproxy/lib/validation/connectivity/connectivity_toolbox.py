...
from duoauthproxy.lib.validation.connectivity.connect import (
    duo,
    ip,
    ldap_operations,
    listen,
...
class ConnectivityTestToolbox(object):
    ...
    def test_ssl_credentials(
        self, key_file_path, cert_file_path, ciphers=None, minimum_tls_version=None
    ):
        ...
        return ssl_operations.can_validate_ssl_creds(
            key_file_path, cert_file_path, ciphers, minimum_tls_version,
        )
    ...
    def test_ldap_search_has_results(self, bound_client, search_dn, filter_object):
        ...
        """
        result = yield ldap_operations.ldap_search_has_results(
            bound_client, search_dn, filter_object
        )
        defer.returnValue(result)
