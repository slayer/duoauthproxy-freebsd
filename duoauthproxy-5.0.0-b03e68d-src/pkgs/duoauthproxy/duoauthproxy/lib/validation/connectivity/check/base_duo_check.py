...
def perform_credentials_check(
    ...
    if ikey and skey and api_host:
        ...
        credentials_check_result = ConfigProblemSkippedTestResult(
            "validate api credentials", "api credentials"
        )

    return credentials_check_result


def _get_duo_api_client(
    api_host, skey="", ikey="", ca_path="", timeout=DEFAULT_TIMEOUT, http_proxy=None
):
    """
    ...
    client = duo_client.Auth(
        ikey, skey, api_host, user_agent=user_agent, ca_certs=ca_path, timeout=timeout
    )
    if http_proxy:
