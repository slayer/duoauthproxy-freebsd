...
from ..connectivity_results import AuthproxyUsingPortSkippedTestResult, ListenResult
...
def can_listen_tcp(port, interface=""):
    ...
    try:
        listener = reactor.listenTCP(
            port=port, factory=connection_server_factory, interface=interface,
        )
...
def can_listen_udp(port, interface=""):
    ...
    try:
        listener = reactor.listenUDP(port=port, protocol=protocol, interface=interface,)
        yield listener.stopListening()
        result = ListenResult(True, port, interface)
    except CannotListenError as e:
...
def _reason_port_is_used(port, interface, exception, listen_type):
    ...
    return ListenResult(
        False, port, interface, port_user=port_user, pid=pid, exception=exception
    )
