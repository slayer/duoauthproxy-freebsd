    ...
    def _verify_callback(self, conn, cert, errno, depth, preverify_ok):
        if preverify_ok and depth == 0:
            try:
                ...
                log.failure("Certificate verification failed.")
                if self.verbose:
                    log.msg(
                        "Failing certificate: "
            ...
            log.error(
                "Certificate verification failed: errno {errno} depth={depth} subject {subject}",
                errno=errno,
                depth=depth,
...
def load_ca_bundle(bundle_file: Union[str, IO[str]]):
    ...
    pattern = re.compile(
        "-----BEGIN CERTIFICATE-----.*?-----END CERTIFICATE-----", re.DOTALL
    )
