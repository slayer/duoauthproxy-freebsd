...
class ConfigProblemSkippedTestResult(BaseResult):
    OUTPUT_MESSAGE = (
        "The Auth Proxy did not run the {test_name} check because of the configuration problem with {config_key}.  "
    ...
    def to_log_output(self, logger):
        logger.warn(
            self.OUTPUT_MESSAGE, test_name=self.test_name, config_key=self.config_key
        )
...
class UnmetPrerequisiteSkippedTestResult(BaseResult):
    OUTPUT_MESSAGE = (
        "The Auth Proxy did not run the {test_name} check because of the problem(s) with the {prereq} check. "
...
class BadProtectedSkeyResult(BaseResult):
    OUTPUT_MESSAGE = (
        "The provided skey_protected could not be decrypted.  "
...
class InvalidConfigKeyProblem(BaseResult):
    ...
    def to_log_output(self, logger):
        logger.error(
            self.MESSAGE, config_key=self.config_key, value=self.provided_value
        )

    def __eq__(self, other):
        return (
            self.config_key == other.config_key
            and self.provided_value == other.provided_value
        )
    ...
    def __str__(self):
        ...
        Returns:
            A string dump of the result
        """
        return "invalid config key {0} with value {1}".format(
            self.config_key, self.provided_value
        )
...
class ListenResult(IndividualTestResult):
    ...
    FAILURE_MESSAGE = (
        "The Auth Proxy will not be able to accept connections on port {port} on {interface}.  "
        "{port} is already in use by another application: {port_user} running with PID: {pid}."
    )

    def __init__(
        self, success, port, interface, port_user=None, pid=None, exception=None
    ):
        """
    ...
    def to_success_log_output(self, logger):
        logger.info(
            self.SUCCESS_MESSAGE, port=self.port, interface=self._interface_string()
        )
...
class ConnectResult(IndividualTestResult):
    SUCCESS_MESSAGE = (
        "The Auth Proxy was able to establish a connection to {host}:{port}."
    )
    FAILURE_MESSAGE = (
        "The Auth Proxy was not able to establish a connection to {host}:{port}."
    )
    ...
    def __str__(self):
        ...
        """
        return "success {0} port {1} host {2} exception {3}".format(
            self.success, self.port, self.host, str(self.exception)
        )


class LdapConnectResult(ConnectResult):
    SUCCESS_MESSAGE = (
        "The Auth Proxy was able to establish an LDAP connection to {host}:{port}."
    )
    FAILURE_MESSAGE = (
        "The Auth Proxy was not able to establish an LDAP connection to {host}:{port}."
    )


class RadiusConnectResult(ConnectResult):
    SUCCESS_MESSAGE = (
        "The Auth Proxy was able to establish a RADIUS connection to {host}:{port}."
    )
    FAILURE_MESSAGE = (
        "We cannot confirm that the Auth Proxy was able to establish a RADIUS connection to {host}:{port}. In the case"
        " of an actual failure this may be due to a misconfigured secret or network issues. This may also happen if the"
        " upstream RADIUS Server does not support the Status-Server message"
    )
    ...
    def to_failure_log_output(self, logger):
        logger.warn(self.FAILURE_MESSAGE, host=self.host, port=self.port)
        logger.debug(self.EXCEPTION_MESSAGE, exception_string=str(self.exception))


class ValidateApiCredentialsResult(IndividualTestResult):
    SUCCESS_MESSAGE = (
        "The Auth Proxy was able to validate the provided API credentials."
    )
    FAILURE_MESSAGE = (
        "The Auth Proxy was not able to validate the provided API credentials. "
    )

    FAILURE_MESSAGE_CHECK_CREDS = "Check that the credentials provided for {api_host} and ikey {ikey} are correct."
    FAILURE_MESSAGE_CHECK_CERTS = (
        "This appears to be because of unreadable or invalid CA certificates passed down by [main]'s "
    ...
    def _log_optional_failure_help(self, logger):
        ...
        elif isinstance(self.exception, IOError) and not isinstance(
            self.exception, socket.error
        ):
            logger.error(self.FAILURE_MESSAGE_CHECK_CERTS)
        else:
            logger.error(
                self.FAILURE_MESSAGE_CHECK_CREDS, api_host=self.host, ikey=self.ikey
            )
    ...
    def __str__(self):
        ...
        return "success {0} host {1} ikey {2} exception {3}".format(
            self.success, self.host, self.ikey, str(self.exception)
        )
...
class DuoPingResult(IndividualTestResult):
    ...
    def __str__(self):
        ...
        return "success {0} api_host {1} exception {2} latency {3}".format(
            self.success, self.api_host, self.latency, str(self.exception)
        )
...
class TimeDriftResult(IndividualTestResult):
    ...
    HIGH_DRIFT_MESSAGE = (
        "The time drift between the Auth Proxy host and Duo is excessively high, at {drift} seconds.  "
    ...
    def __str__(self):
        ...
        return "success {0} drift {1} exception {2}".format(
            self.success, self.drift, self.exception
        )
...
class SslFileResult(IndividualTestResult):
    ...
    def __str__(self):
        ...
        return "success {0} file_path {1} exception {2}".format(
            self.success, self.file_path, str(self.exception)
        )


class SslKeyFileResult(SslFileResult):
    SUCCESS_MESSAGE = (
        "The Auth Proxy was able to validate the SSL private key at {file_path}."
    )
    FAILURE_MESSAGE = (
        "The Auth Proxy was not able to validate the SSL private key at {file_path}.  "
...
class SslCertFileResult(SslFileResult):
    SUCCESS_MESSAGE = (
        "The Auth Proxy was able to validate the SSL certificate data at {file_path}."
    )
    FAILURE_MESSAGE = (
        "The Auth Proxy was not able to validate the SSL certificate at {file_path}. "
...
class SslContextResult(IndividualTestResult):
    ...
    FAILURE_MESSAGE_MISMATCHED = (
        "This is because the private key doesn't appear to match the provided certificate. "
    ...
    FAILURE_MESSAGE_BAD_CIPHERS = (
        "This is because the specified cipher list {cipher_list} is invalid. "
        "Please ensure all your ciphers are spelled correctly and are properly delimited with colons."
    )
    FAILURE_MESSAGE_BAD_TLS = (
        "This is because the specified minimum_tls_version { } is invalid. "
    ...
    def _log_optional_failure_help(self, logger):
        ...
        if (
            err_msg == "SSL_CTX_check_private_key"
            or err_msg == "X509_check_private_key"
        ):
    ...
    def __str__(self):
        ...
        return "success {0} cipher list {1} exception {2}".format(
            self.success, self.cipher_list, str(self.exception)
        )
...
class LdapBindResult(IndividualTestResult):
    ...
    FAILURE_MESSAGE_CREDS = (
        "Please ensure that the provided service account credentials are correct."
    )
    FAILURE_MESSAGE_CERTS = (
        "Please ensure that the provided SSL CA certs file matches what was exported from your domain controller. "
    ...
    def __str__(self):
        ...
        return "success {0} bind username {1} exception {2}".format(
            self.success, self.bind_username, str(self.exception)
        )


class LdapSearchResult(IndividualTestResult):
    SUCCESS_MESSAGE = (
        "The Auth Proxy got results searching with its basic user filter.  "
        "It will likely be able to find specific users during authentication."
    )
    NO_RESULTS_MESSAGE = (
        "The Auth Proxy did not get results searching for users in DN {search_dn} using the filter {filter}.  "
    ...
    FAILED_SEARCH_MESSAGE = (
        "The Auth Proxy got an error searching the LDAP DN {search_dn}."
    )
    ...
    def to_failure_log_output(self, logger):
        if self.exception:
            ...
            logger.error(
                ...
                search_dn=self.search_dn,
                filter=self.filter_object.asText(),
            )
    ...
    def __str__(self):
        ...
        return "success {0} search dn {1} filter {2} exception {3}".format(
            self.success, self.search_dn, str(self.filter_object), str(self.exception)
        )
...
class LdapFilterParseResult(IndividualTestResult):
    ...
    FAILURE_MESSAGE = (
        "The Auth Proxy was not able to parse the LDAP filter {filter_text}.  "
    ...
    def __eq__(self, other):
        return self.success == other.success and self.filter_text == other.filter_text
    ...
    def __str__(self):
        ...
        Returns:
            A string dump of the result
        """
        return "success {0} filter {1}".format(self.success, self.filter_text)
...
class LdapHostResult(CompositeTestResult):
    ...
    def is_successful(self):
        return (
            self.tcp_connect_result.is_successful()
...
class SslResult(CompositeTestResult):
    SUCCESS_MESSAGE = (
        "The Auth Proxy was able to create an SSL context with the given certificate and private key. "
        "It will most likely be able to use these credentials to create and maintain SSL-based connections such as LDAPS."
    )
    FAILURE_MESSAGE = (
        "The Auth Proxy was not able to create an SSL context with the given certificate and private key. "
    ...
    def __eq__(self, other):
        return (
            self.key_result == other.key_result
...
class ConfigCheckResult(CompositeTestResult):
    ...
    def __str__(self):
        ...
        """
        return "\n problems {0}".format(
            str([str(problem) for problem in self.problems])
        )
...
class RadiusServerResult(CompositeTestResult):
    ...
    def is_successful(self):
        return (
            self.config_result.is_successful()
    ...
    def __eq__(self, other):
        return (
            self.config_result == other.config_result
...
class CloudResult(CompositeTestResult):
    ...
    def is_successful(self):
        return (
            self.ping_result.is_successful() and self.time_drift_result.is_successful()
        )
    ...
    def __eq__(self, other):
        return (
            self.ping_result == other.ping_result
            and self.time_drift_result == other.time_drift_result
        )
...
class AdClientConnectivityResult(CompositeTestResult):
    ...
    def is_successful(self):
        hosts_good = all(
            [host_result.is_successful() for host_result in self.ldap_host_results]
        )
...
class LdapServerResult(CompositeTestResult):
    ...
    def __init__(
        self, ping_result, time_drift_result, validate_result, ssl_result, listen_result
    ):
        """
    ...
    def is_successful(self):
        return (
            self.ping_result.is_successful()
            and self.time_drift_result.is_successful()
            and self.validate_result.is_successful()
...
class MainSectionResult(CompositeTestResult):
    ...
    def is_successful(self):
        return (
            self.certs_result.is_successful() and self.http_proxy_result.is_successful()
        )
    ...
    def __eq__(self, other):
        return (
            self.certs_result == other.certs_result
    ...
    def __str__(self):
        ...
        """
        return "\n certs: {0} \n http_proxy: {1}".format(
            str(self.certs_result), str(self.http_proxy_result)
        )
