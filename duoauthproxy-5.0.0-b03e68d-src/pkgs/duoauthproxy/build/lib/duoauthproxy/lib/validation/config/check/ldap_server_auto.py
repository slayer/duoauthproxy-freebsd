...
def _check_config_values(config, toolbox):
    ...
    base.add_dynamic_keys_to_test_resolver(
        config, config_value_tests, dynamic_key_tests
    )
...
def _check_config_dependencies(config, toolbox):
        ...
        for key in ("ssl_key_path", "ssl_cert_path"):
            if not toolbox.test_config_has_value(config, key):
                dependency_problems.append(
                    config_results.MissingKey(
                        level=config_results.ConfigResultLevel.Warning, key=key
                    )
                )

    # ssl_key_path and ssl_cert_path are both or neither
    if toolbox.test_config_has_value(
        config, "ssl_key_path"
    ...
    if (
        toolbox.test_is_bool(config, "allow_unlimited_binds")
        and config.get_bool("allow_unlimited_binds", False)
        and toolbox.test_is_bool(config, "exempt_primary_bind")
        and config.get_bool("exempt_primary_bind", False)
    ):
        dependency_problems.append(
            config_results.IncompatibleKeys(
                key1="allow_unlimited_binds", key2="exempt_primary_bind"
            )
        )
