...
class DrpcServerModule(Service, object):
    ...
    @staticmethod
    def make_duo_client(
        duo_creds, host, port=443, client_type=duo_async.DirectorySyncDuoClient
    ):
        return client_type(host=host, duo_creds=duo_creds, port=port,)
    ...
    def register_drpc_call_provider(self, provider_id, call_provider):
        ...
        required_methods = ["get_drpc_calls", "register_new_parameters"]
        for method in required_methods:
            if not hasattr(call_provider, method):
                raise InvalidPlugin(
                    "DRPC call provider missing required method. Provider id: {}. Method name: {}".format(
                        provider_id, method
                    )
                )
    ...
    def startService(self):
        ...
        self._check_connection_lc = LoopingCall(
            self._check_connection, clock=self.reactor
        )
    ...
    def _check_connection(self):
        ...
        if not self.rpc_server:
            ...
            self.rpc_server.factory.idle_rpc_timeout is not None
            ...
            if last_rpc > self.rpc_server.factory.idle_rpc_timeout:
                ...
                try:
                    ...
                    log.msg(
                        "Attempted to forcibly disconnect the transport but was unable. Transport likely already disconnected. Exception: {}".format(
                            e
                        )
                    )
        ...
        if disconnected:
            ...
            try:
                ...
                if is_fatal_error(e):
                    ...
                    log.msg(
                        "Error connecting to service: {0}. Will retry again in {1} seconds.".format(
...
def is_fatal_error(error: Exception) -> bool:
    if isinstance(error, duo_async.DuoAPIError):
        if (
            ...
            and "code" in error.info
            and error.info["code"] in FATAL_ERRORS
        ):
            return True
    return False
