...
FAIL_AUTH_MSG = "Denied Duo login on auth failure"
FAIL_AUTH_TIMEOUT_MSG = "Denied Duo login due to timeout"
...
class DuoAPITimeoutError(DuoAPIError):
    pass
...
class DuoAPIMalformedResponseError(DuoAPIFailOpenError):
    def __init__(self, key):
        msg = "Malformed API Response - invalid or missing '%s'" % key
...
def should_server_fail_open(server_section_failmode, is_fail_open):
    ...
    """
    return server_section_failmode == FAILMODE_SAFE and (
        is_fail_open or PrimaryOnlyManager.is_primary_only_enabled()
    )
...
def bad_signature(response, status):
    return (
        "code" in response and str(response["code"]) == "40103" and str(status) == "401"
    )
...
class BaseDuoClient(abc.ABC):
    ...
    def call(
        ...
        code, data, headers = yield self._request(
            *request_args, force_fail_closed=force_fail_closed, timeout=timeout
        )
...
class AuthDuoClient(BaseDuoClient):
    ...
    def _request(
        ...
        **kwargs
        ...
        d = self.requestor(method, url, body, headers, timeout=timeout, **kwargs)

        def err_func(err):
            if force_fail_closed:
                if err.check(defer.TimeoutError):
                    raise DuoAPITimeoutError("API Request Failed: %r" % err.value)
                else:
                    raise DuoAPIFailClosedError("API Request Failed: %r" % err.value)
            else:
                raise DuoAPIFailOpenError("API Request Failed: %r" % err.value)
    ...
    def preauth(self, username, client_ip, failmode=FAILMODE_UNKNOWN):
        ...
        preauth_res = yield self.call(
            "POST", "/rest/v1/preauth", params, signature_version=1
        )
    ...
    def auth(self, username, passcode, client_ip):
        ...
        if len(passcode) > MAX_FACTOR_OR_PASSCODE_LENGTH:
            raise DuoAPIError(
                "Factor or passcode length cannot exceed {}".format(
                    MAX_FACTOR_OR_PASSCODE_LENGTH
                )
            )

        params = dict(user=username, factor="auto", auto=passcode, ipaddr=client_ip)
    ...
    def proxy_init(self, username):
        init_res = yield self.call(
            "POST", "/rest/v1/tx/proxy_init", dict(user=username), signature_version=1
        )
    ...
    def proxy_finish(self, auth_cookie):
        finish_res = yield self.call(
            "POST",
            "/rest/v1/tx/proxy_finish",
            dict(auth_cookie=auth_cookie),
            signature_version=1,
        )
...
class HTTPClientDRPC(twistedHTTPClient, abc.ABC):
    ...
    def __init__(self, duo_client, server_module, drpc_path, *args, **kwargs):
    ...
    def make_drpc_server_factory(self, time_offset, response):
        ...
        raise NotImplementedError(
            "Children class must implement this function for their specific DRPC version and use case"
        )
    ...
    def connectionMade(self):
        ...
        self.sendHeader(b"Host", self.factory.headers.get(b"host", self.factory.host))
        self.sendHeader(b"User-Agent", self.factory.agent)
        auth = self.duo_client._sign_request(
            method=method, uri=path, params={}, date=date,
        )
    ...
    def sendCommand(self, command, path):
        self.transport.writeSequence(
            [command.encode(), b" ", path.encode(), b" HTTP/1.1\r\n"]
        )
    ...
    def _build_error_message(response, status):
        ...
        error_message = "Unexpected HTTP status {status}. Error message: {err_msg}".format(
            status=status, err_msg=response_message
        )
