...
from duoauthproxy.lib import const, ldap, log, secret_storage, util
from duoauthproxy.lib.const import AD_AUTH_TYPE_PLAIN
from duoauthproxy.modules.drpc_plugins import ldap_base
from duoauthproxy.types import BytesLDAPEntry, LDAPAttribute
...
MSDS_PRINCIPAL_NAME_MISSING = (
    "Required attribute msDS-PrincipalName is missing for user {}"
)
...
class LdapSsoDrpcPlugin(ldap_base.BaseLdapDrpcPlugin):
    ...
    def do_ldap_authentication(
        ...
        try:
            ...
            log.msg(msg)
            raise drpc.CallError(ERR_LDAP_PW_DECRYPT_FAILED, {"error": str(e),})
    ...
    def perform_authentication(
        ...
        if not password:
            defer.returnValue(
                {"success": False, "msg": AUTH_FAILED, "exceptions": {},}
            )
    ...
    def connection_failures_for_all_servers(self, servers, exceptions):
        ...
        for host in exceptions:
            if exceptions[host]["error"] not in [
                ldap_base.ERR_LDAP_TIMEOUT,
                ldap_base.ERR_LDAP_CONNECTION_FAILED,
                ldap_base.ERR_LDAP_HOSTNAME_RESOLUTION_FAILED,
            ]:
                return False
    ...
    def authenticate_against_server(
        ...
        * username_attribute: attribute within AD that we will compare the
        ...
        username_attributes = (
            username_attributes if username_attributes else ["samaccountname"]
        )
        ...
        try:
            ...
            try:
                ...
                if timeout_dc.active():
                    ...
                    raise drpc.CallError(
                        ldap_base.ERR_LDAP_BIND_FAILED, {"error": str(e),}
                    )
                else:
                    log.failure(ldap_base.ERR_LDAP_TIMEOUT)
                    raise drpc.CallError(
                        ldap_base.ERR_LDAP_TIMEOUT, {"during": "bind", "error": str(e),}
                    )
            ...
            try:
                ...
                attributes_to_fetch = {
                    str(attr).lower() for attr in username_attributes
                }
                ...
                for base_dn in base_dns:
                    result = yield cl.perform_search(
                        base_dn, filterObject, attributes=tuple(attributes_to_fetch)
                    )
                ...
                raise drpc.CallError(
                    ldap_base.ERR_LDAP_BAD_AD_CONFIGURATION, {"error": str(e),}
                )
            except Exception as e:
                log.failure(
                    "{msg} for {username}",
                ...
                raise drpc.CallError(
                    ldap_base.ERR_LDAP_SEARCH_FAILED, {"error": str(e),}
                )

            if len(search_hits) == 0:
                err_msg = AUTH_INVALID_USER
                err = drpc.CallError(
                    ldap_base.ERR_LDAP_SEARCH_FAILED, {"error": err_msg,}
                )
                log.error(
                    "{msg} {username}. Error: {error}",
                    msg=err_msg,
            ...
            log.msg(
                "Found {username} with attributes {atts}".format(
            ...
            if auth_type != AD_AUTH_TYPE_PLAIN:
                ...
                msds_principalname = list(user_result.get("msDS-PrincipalName"))[
                    0
                ].decode("utf8")
        ...
        log.msg(
            "Authentication succeeded for user {username} ({dn})".format(
                username=username, dn=user_full_dn
            )
        )
    ...
    def do_fetch_ldap_attributes(
        ...
        for server in servers:
            ...
            try:
                ...
                exceptions[host] = {"error": e.error, "error_args": e.error_args}
    ...
    def fetch_ldap_attributes_from_server(
        ...
        try:
            ...
            try:
                ...
                if timeout_dc.active():
                    log.failure(ldap_base.ERR_LDAP_BIND_FAILED)
                    raise drpc.CallError(
                        ldap_base.ERR_LDAP_BIND_FAILED, {"error": six.text_type(e)}
                    )
                else:
                    log.failure(ldap_base.ERR_LDAP_TIMEOUT)
                    raise drpc.CallError(
                        ldap_base.ERR_LDAP_TIMEOUT,
                        {"during": "bind", "error": six.text_type(e)},
                    )
            ...
            try:
                ...
                raise drpc.CallError(
                    ldap_base.ERR_LDAP_SEARCH_FAILED, {"error": six.text_type(e)}
                )
        finally:
            # Clean up networking
            if timeout_dc.active():
        ...
        logged_atts = transform_result(
            result, desired_attributes, value_transform=self.to_unicode_ignore_errors
        )
        log.msg(
            "For user dn {dn}, found attributes {atts}".format(
    ...
    def do_ldap_health_check(
        ...
        try:
            try:
                ...
                raise drpc.CallError(drpc_error, {"error": str(e),})
            except ldaperrors.LDAPInvalidCredentials as e:
                raise drpc.CallError(ERR_LDAP_BIND_INVALID_CREDS, {"error": str(e),})
            except Exception as e:
            ...
            filter_obj = (
                ldapfilter.parseFilter(filter_text.encode("utf-8"))
                if filter_text
                else None
            )
            try:
                ...
                raise drpc.CallError(
                    ERR_LDAP_INVALID_BASE_DN, {"error": str(e), "base_dn": str(base_dn)}
                )
            except ldap.client.ConnectionNotProperlyBoundError as e:
                log.failure(ldap_base.ERR_LDAP_SEARCH_FAILED_BAD_BIND)
                raise drpc.CallError(
                    ldap_base.ERR_LDAP_SEARCH_FAILED_BAD_BIND, {"error": str(e),}
                )
            except Exception as e:
                # T76043: add better/more specific error handling so we can tell
                # Gary why the search failed
            ...
            try:
                ...
                log.failure("Error cleaning up connection to host {}".format(host))
    ...
    def do_get_proxy_counter(self):
        ...
        try:
            ...
            raise drpc.CallError(
                ERR_COUNTER_FAILURE, {"during": "get_proxy_counter", "error": str(e)}
            )
    ...
    def get_creds_for_ldap_idp(self, rikey, auth_type):
        try:
            ...
            raise drpc.CallError(ERR_LDAP_SSO_MISSING_RIKEY, {"error": str(e),})
...
def determine_matched_attributes(
    target_username: str, candidate_attributes: List[str], user_result
) -> List[str]:
    """
    Args:
        target_username (str):
    ...
    for attribute in candidate_attributes:
        ...
        result_attributes_as_strings: List[str] = [
            attr.decode("utf8") for attr in result_attributes_as_bytes
        ]
