...
from duoauthproxy.lib import const, ldap, log, util
...
ERR_LDAP_SEARCH_FAILED_BAD_BIND = "LDAP search failed due to bad bind"
...
class BaseLdapDrpcPlugin(abc.ABC):
    ...
    def _get_client(
        ...
        try:
            ...
            raise drpc_exceptions.CallError(
                ERR_LDAP_CONFIGURATION_FAILED, {"error": str(e),}
            )
        ...
        try:
            ...
            if isinstance(e.underlying_exception, DNSLookupError):
                ...
                raise drpc_exceptions.CallError(
                    ERR_LDAP_HOSTNAME_RESOLUTION_FAILED, {"error": str(e),}
                )
            else:
                log.failure(ERR_LDAP_CONNECTION_FAILED)
                raise drpc_exceptions.CallError(
                    ERR_LDAP_CONNECTION_FAILED, {"error": str(e),}
                )
        except Exception as e:
            log.failure(ERR_LDAP_CONNECTION_FAILED)
            raise drpc_exceptions.CallError(
    ...
    def _verify_ldap_config_args(
        self, bind_dn: str, bind_pw: str, auth_type: str, transport_type: str
    ) -> None:
        ...
        if auth_type != const.AD_AUTH_TYPE_SSPI and (bind_dn == "" or bind_pw == ""):
            ...
            log.error(
                "{msg}. Error: {error}", msg=ERR_LDAP_CONFIGURATION_FAILED, error=e
            )
            raise e
