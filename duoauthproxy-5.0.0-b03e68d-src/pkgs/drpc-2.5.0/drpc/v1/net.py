...
class Protocol(basic.Int32StringReceiver,
    ...
    def send_msg(self, obj, serialize_obj=True):
        ...
        * serialize_obj: If False, expect obj to be an
        ...
        sig_bytes = self.jsonify({
            'sig': hmac_sha256(self.factory.skey, data_bytes, sig_time),
            'time': sig_time,
        }).encode()
        ...
        self.factory.log_debug(
            ...
            data_length=len(data_bytes), msg_id=msg_id,
        )
    ...
    def verify_sig(self, data_bytes, sig, now=None):
        ...
        if abs(sig_time - now) > self.SIG_WINDOW:
            return False

        actual_sig = hmac_sha256(self.factory.skey, data_bytes, str(sig_time))
...
def hmac_sha256(skey, data_bytes, sig_time):
    """
    Return the signature string for the given time and skey.
    """
    data_bytes = base64.b64encode(data_bytes)
    time_bytes = bytes(str(sig_time), 'utf8')
    canon_bytes = data_bytes + b'|' + time_bytes
    return hmac.HMAC(skey, canon_bytes, hashlib.sha256).hexdigest()
...
class BaseFactory(protocol.Factory, object):
    def __init__(self, ikey, skey, time_offset=0,
                 streaming_max_length=DEFAULT_STREAMING_MAX_LENGTH,
                 enable_unzlib=False,
                 auto_zlib_min_length=None,
