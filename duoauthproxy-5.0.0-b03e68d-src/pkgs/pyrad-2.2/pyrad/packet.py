...
import hmac
# BEGIN DUO EDIT @iraja: Wrap the md5_constructor with `used_for_security=Depends`.
...
# BEGIN DUO EDIT @iraja: Wrap the md5_constructor with `used_for_security=Depends`.
import functools
md5_constructor = functools.partial(md5_constructor, used_for_security = Depends)
# END DUO EDIT
import six
...
class Packet(OrderedDict):
    ...
    def __init__(self, code=0, id=None, secret=six.b(''), authenticator=None,
        ...
        # BEGIN DUO EDIT
        # See https://github.com/pyradius/pyrad/pull/144/files
    ...
    def _refresh_message_authenticator(self):
        # BEGIN DUO EDIT @iraja: specifically pass in the wrapped md5
        # hmac_constructor = hmac.new(self.secret)
        hmac_constructor = hmac.new(self.secret, None, md5_constructor)
        # END DUO EDIT
    ...
    def verify_message_authenticator(self, secret=None,
        ...
        # Message-Authenticator. This is necessary since the attribute order
        # is changed by _PktEncodeAttributes if multiple instances of an
        # attribute exist on the packet and are not adjacent to each other
        ...
        # BEGIN DUO EDIT @iraja
        # hmac_constructor = hmac.new(key)
        hmac_constructor = hmac.new(key, None, md5_constructor)
        # END DUO EDIT
    ...
    def AddAttribute(self, key, value):
        ...
        """
	# BEGIN DUO EDIT @iraja: In the AP, we pass in keys that do not necessarily
	# map to values in the dictionary so just store and encode what we pass in.
        #attr = self.dict.attributes[key]
    ...
    def get(self, key, failobj=None):
        # XXX DUO MOD this new get() method only handles keys that return
        # XXX falsey values. It raises an exception with non-existant keys.
        # XXX It was causing bugs in our usage because get() methods
        # XXX traditionally don't throw exceptions for missing keys and
        # XXX we were relying on the default being returned instead.
        # XXX I believe this was an oversight by the pyrad team
        # XXX so I made it act more like the traditional get() method.
        try:
    ...
    def VerifyReply(self, reply, rawreply=None):
        ...
        # DUO EDIT @mbishop
        # This fix is taken from the upstream commit
        # https://github.com/pyradius/pyrad/commit/2c9a455784a636148400d79885e8819169032831
        # which was made in response to an issue we found when having multiple
        # entries for the same attribute.  We wanted this change in for the
        # 4.1RC but in the future we should be able to remove this when pyrad
        # releases version 2.4
    ...
    def _PktEncodeAttributes(self):
        ...
        for (code, datalst) in self.items():
            #XXX DUO MOD use our safe checker
            if self.is_attribute_a_tlv(code):
    ...
    def _PktDecodeVendorAttribute(self, data):
        ...
        try:
            #XXX DUO MOD use our safe checker
            if self.is_attribute_a_tlv((vendor, type)):
    ...
    def DecodePacket(self, packet):
        ...
        while packet:
            ...
            if key == 26:
                ...
                self.setdefault(key, []).append(value)
            #XXX DUO MOD use our safe checker
            elif self.is_attribute_a_tlv(key):
    ...
    # XXX DUO MOD We need to add a safe function for checking if an attribute
    # XXX is a TLV. Currently the pyrad code assumes we know about every key
    # XXX in our dictionary, but that's not true for us. In order to support
    # XXX even keys we don't know about we just assume any key we don't know
    # XXX about is not a TLV. This should be safe because the default will
    # XXX then be to just leave the raw bytes alone and place them on the
    # XXX packet.
    def is_attribute_a_tlv(self, key) -> bool:
        try:
            return self.dict.attributes[self._DecodeKey(key)].type == 'tlv'
        except KeyError:
            return False
    # XXX END DUO MOD
...
class AuthPacket(Packet):
    ...
    # BEGIN DUO EDIT @iraja: Allow passing of an optional codec parameter
    # def PwDecrypt(self, password):
    # END DUO EDIT
    def PwDecrypt(self, password, pw_codec='utf-8'):
        ...
        return pw.decode(pw_codec)

    # BEGIN DUO EDIT @iraja: Allow passing of an optional codec parameter
    # def PwCrypt(self, password):
    # END DUO EDIT
    def PwCrypt(self, password, pw_codec='utf-8'):
        """Obfuscate password.
        ...
        if isinstance(password, six.text_type):
            ...
            # END DUO EDIT
            password = password.encode(pw_codec)
