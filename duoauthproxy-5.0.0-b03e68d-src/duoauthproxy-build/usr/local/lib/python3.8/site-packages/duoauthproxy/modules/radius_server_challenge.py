...
from ..lib.base import AuthResult, ServerModule
from ..lib.radius.base import MS_CHAP2_REQUEST_ATTRS
from ..lib.radius.challenge import (
    RADIUS_CHALLENGE_PROMPT_FORMATS,
    ChallengeResponseRadiusServer,
)
...
def _format_console(preauth_res: Dict[str, Any]):
    return preauth_res["prompt"]


def _format_html(preauth_res: Dict[str, Any]):
    lines = (
        xml.sax.saxutils.escape(x.strip()) for x in preauth_res["prompt"].splitlines()
    )
    return "<br>".join(lines)


def _format_short(preauth_res: Dict[str, Any]):
    factors = preauth_res["factors"]
    if factors:
        sorted_factors = (
            "'%s'" % factors[k] for k in sorted(factors.keys()) if k != "default"
        )
        msg = "Choose a secondary factor from (%s) or enter passcode:" % ", ".join(
            sorted_factors
        )
        if len(msg) > 253:
...
class DuoTextChallengeRadiusServer(ChallengeResponseRadiusServer):
    ...
    def __init__(self, prompt_format, enroll_challenge, failmode, duo_client, **kwargs):
    ...
    def _format_msg(self, preauth_res: Dict[str, Any]):
    ...
    def _create_proxy_response(
        self, request: _ProxyRequest, primary_res: AuthResult, msg: str = None
    ):
        ...
        if primary_res.success:
            ...
            response = self.create_accept_packet(request, msg, primary_res.radius_attrs)
        else:
            self.log_request(request, "Sending access reject packet")
            response = self.create_reject_packet(request, msg, primary_res.radius_attrs)

        return response

    @defer.inlineCallbacks
    def _preauth_cpw_response(
        self, request: _ProxyRequest, preauth_res: Dict[str, Any]
    ):
        """
        ...
        Returns:
            packet.Packet: a RADIUS response to send to the client.  This could
            be a RADIUS rejection message if preauth failed, a RADIUS challenge,
            or a RADIUS access-accept.
        """
    ...
    def _preauth_response(
        ...
        if preauth_res["result"] == duo_async.API_RESULT_ALLOW:
            ...
            return self._create_proxy_response(
                request, primary_res, preauth_res["status"]
            )
    ...
    @defer.inlineCallbacks
    def preauth(self, request: _ProxyRequest):
        """
        ...
        try:
            ...
            if "status" in preauth_res:
                self.log_request(
                    request,
                    "Duo preauth returned '%s': '%s'"
                    % (preauth_res["result"], preauth_res["status"]),
                )
            else:
                self.log_request(
                    request, "Duo preauth returned '%s'" % preauth_res["result"]
                )
        defer.returnValue(preauth_res)
    ...
    def get_initial_response(self, request: _ProxyRequest):
        ...
        Returns:
            packet.Packet: the RADIUS message to send to the client.
        """
        ...
        if request.password is None and not self._is_chap2_req(request):
        ...
        if not primary_res.success:
            ...
            if "MS-CHAP-Error" in primary_res.radius_attrs:
                ...
                if m is not None and int(m.group("E")) == self.ERROR_PASSWORD_EXPIRED:
                    ...
                    if preauth_res["result"] == duo_async.API_RESULT_DENY:
                        ...
                        self.log_request(
                            request, "Sending auth deny access reject packet"
                        )
                        radius_attrs = {"MS-CHAP-Error": self.MS_AUTH_FAILED_ERROR}
                        defer.returnValue(
                            self.create_reject_packet(
                                request, radius_attrs=radius_attrs
                            )
                        )
                        ...
                        if self.enroll_challenge:
                            ...
                            challenge_packet = self.create_challenge(
                                request, preauth_res["status"], state
                            )
                            msg_challenge = log.AUTH_ENROLL_MSG
                            self.log_request(
                                request, "Sending enrollment challenge packet"
                            )
                            log.auth_standard(
                                msg=msg_challenge,
                                username=request.username,
                        ...
                        msg = preauth_res["status"]
                        radius_attrs = {"MS-CHAP-Error": self.MS_AUTH_FAILED_ERROR}
                        reject_packet = self.create_reject_packet(
                            request, msg, radius_attrs
                        )
        ...
        preauth_res = yield self.preauth(request)
        response_packet = self._preauth_response(request, primary_res, preauth_res)
        defer.returnValue(response_packet)

    def handle_enroll_challenge(
        self, request: _ProxyRequest, state: Dict[str, Any]
    ) -> packet.Packet:
    ...
    @defer.inlineCallbacks
    def handle_cpw_auth_challenge(self, request: _ProxyRequest, state: Dict[str, Any]):
        """
        Handle the user's auth challenge response, in the change password case

        Returns:
            packet.Packet: The response to be sent to the client
        """
    ...
    def handle_auth_challenge(
        self, request: _ProxyRequest, state: Dict[str, Any], cpw_auth=False
    ):
        """
        ...
        Returns:
            packet.Packet: the RADIUS response to be sent to the client.
        """
        ...
        # do auth request
        try:
            auth_res = yield self.client.auth(
                request.username, passcode, request.client_ip
            )
        except duo_async.DuoAPIError as e:
            ...
            if duo_async.should_server_fail_open(self.failmode, e.fail_open):
                ...
                response_packet = self.create_reject_packet(
                    request, duo_async.FAILMODE_SECURE_MSG, radius_attrs
                )
                log.auth_standard(
        ...
        if self.SMSREFRESH_RE.match(passcode):
            response_packet = self.create_challenge(request, state["prompt"], state)
