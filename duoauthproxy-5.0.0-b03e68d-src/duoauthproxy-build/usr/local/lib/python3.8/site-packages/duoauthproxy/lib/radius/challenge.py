...
class ChallengeResponseRadiusServer(SimpleRadiusServer):
    ...
    def _create_challenge_id(self) -> bytes:
    ...
    def create_challenge(
        ...
        radius_attrs = {"State": challenge_id}

        response_packet = self._create_response_packet(
            request, packet.AccessChallenge, msg, radius_attrs
        )

        # store challenge state so we can look it up
        self.store_challenge_state(request, challenge_id, state=state)

        return response_packet

    def store_challenge_state(
        self, request: _ProxyRequest, challenge_id: bytes, state: Dict[str, Any] = None
    ) -> _ChallengeState:
        """
        ...
        challenge.
        """
        challenge_state = _ChallengeState(request, challenge_id, state=state)
        challenge_state.cleanup_dc = reactor.callLater(
            self.CHALLENGE_WAIT, self._cleanup_challenge, challenge_state
        )
    ...
    def _find_challenge(self, request: _ProxyRequest) -> Optional[_ChallengeState]:
    ...
    def get_response(self, request: _ProxyRequest):
        ...
        try:
            ...
            if challenge_state:
                ...
                # build response
                ret = yield self.get_challenge_response(request, challenge_state.state)
            else:
    ...
    @defer.inlineCallbacks
    def get_initial_response(self, request: _ProxyRequest) -> packet.Packet:
