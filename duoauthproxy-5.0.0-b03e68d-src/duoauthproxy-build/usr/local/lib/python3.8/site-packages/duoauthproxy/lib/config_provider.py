...
from .config_error import ConfigError, ConfigMissingError
...
class ConfigProvider:
    ...
    def __init__(self, ini_string, source=None, apply_overrides=True):
        ...
        if duplicates:
            raise ConfigError(
                "Configuration loaded from {0} has duplicate sections: {1}".format(
                    source, duplicates
                )
            )
        ...
        try:
            ...
            raise ConfigError(
                "Config is unparseable or contains bad comment: {}".format(e)
            )
        except DuplicateOptionError as e:
            raise ConfigError(
                "Configuration has duplicated keys in the given section: {}".format(e)
            )
    ...
    def _add_encrypted_value(self, section, option):
        ...
        if not protect.PROTECT_ENABLED:
            raise ConfigError(
                "Attempting to encrypt on a system that doesn't support it"
            )
    ...
    def _remove_unencrypted_value(self, section, option):
        ...
        try:
            ...
            if not option_exists:
                raise ConfigError(
                    "Failed to remove option: {0} in section: {1}".format(
                        option, section
                    )
                )
        except NoSectionError:
...
def _get_local_config(config_filename=None, apply_overrides=True):
    ...
    try:
        ...
        with open(config_filename, "r") as config_file:
            configuration = ConfigProvider.from_ini_file(
                config_file, os.path.abspath(config_filename), apply_overrides
            )
    except IOError:
        raise ConfigMissingError(
            "Config file missing or unreadable: {0}".format(config_filename)
        )
    ...
    def get_int(self, key, default=None):
        ...
        if value is None:
            ...
            try:
                ...
                raise ConfigError(
                    "Invalid value for configuration item: '%s' must be an integer"
                    % key
                )
        ...
        if value is None:
            ...
            if value.lower() == "true":
                ...
                try:
                    ...
                    raise ConfigError(
                        "Invalid value for configuration item: '%s' must be a boolean value"
                        % key
                    )
    ...
    def get_enum(self, key, values, default=None, transform=lambda x: x):
        ...
        if value is None:
            ...
            if value not in values:
                raise ConfigError(
                    "Invalid value for configuration item: '%s' must be one of %s"
