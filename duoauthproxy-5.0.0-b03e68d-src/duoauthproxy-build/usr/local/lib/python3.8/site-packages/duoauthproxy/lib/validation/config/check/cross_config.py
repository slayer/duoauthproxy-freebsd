from duoauthproxy.lib import config_provider, ip_util, util
        ...
        if "http_proxy_host" in sections.get("main"):
            for section in sections:
                if section.startswith("http_proxy"):
                    problems.append(
                        KeyIncompatibleWithSection(key1="http_proxy_host", key2=section)
                    )
...
def _get_section_radius_listeners(config, section_name):
    ...
    for ip_key in util.get_dynamic_keys(config, "radius_ip"):
        try:
            ...
            listeners.append(
                RadiusListener(ip_networks, ip_str, port, interface, section_name)
            )
        except Exception:
...
class RadiusListener(object):
    ...
    def conflicts_with(self, listener2):
        ...
        if self.port == listener2.port:
            if (
                self.interface == "*"
                or listener2.interface == "*"
                or self.interface == listener2.interface
            ):
        ...
        for l2 in listeners:
            if l1.conflicts_with(l2):
                ...
                if l1.section == l2.section:
                    problems.append(
                        SameSectionPortCollision(section=l1.section, port=l1.port)
                    )
                else:
...
def check_server_client_mapping(config):
    ...
    for section, section_config in config.get_all_sections().items():
        ...
        if section.startswith("radius_server_duo_only") and "client" in section_config:
            problems.append(
                IneffectiveConfig(
                    key="client", section=section, condition="in this type of section"
                )
            )
        ...
        if client_name:
            if client_name not in clients:
                problems.append(
                    InvalidClientServerMapping(client_name=client_name, section=section)
                )

            if not client_name.startswith("ad_client") and section.startswith(
                "ldap_server_auto"
            ):
                problems.append(
                    InvalidClientServerMapping(client_name=client_name, section=section)
    ...
    for section, section_config in config.get_all_sections().items():
        if "pass_through_attr_names" in section_config:
            ...
            if client_name and not client_name.startswith("radius_client"):
                problems.append(
                    KeyInvalidWithClient(
                        key="pass_through_attr_names", client=client_name
                    )
                )
...
def check_minimum_tls_version_with_fips_mode(config):
    ...
    if main_config and main_config.get_bool("fips_mode", False):
        for section, section_config in config.get_all_sections().items():
            if section.startswith("ldap_server_auto") or section.startswith(
                "radius_server_eap"
            ):
