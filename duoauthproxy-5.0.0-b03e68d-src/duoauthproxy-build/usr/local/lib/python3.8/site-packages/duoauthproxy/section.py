...
from .lib import base, config_error, const, log
...
def parse_sections(config, is_logging_insecure):
    ...
    for section_name_raw in config.list_sections():
        ...
        # Insecure logging flag is not allowed to be manually set in the config by users
        if "is_logging_insecure" in section_config:
            log.msg(
                "Ignoring invalid config option 'is_logging_insecure' in module '%s'"
                % section_name_raw
            )
            section_config["is_logging_insecure"] = "False"

        # Add the insecure logging and debug options if the insecure logging flag was set
...
class Section(object):
    ...
    def validate(self):
        try:
            if (
                not isinstance(self.module, types.ModuleType)
                or self.module_type == "other"
            ):
                raise Exception()
...
class ModuleFactory(object):
    ...
    def make_modules(self):
        ...
        for t in self.sections:
            if t not in self.module_factories:
                raise config_error.ConfigError(
                    "could not find factory method for module type {0}".format(t)
                )
        ...
        new_modules["server"] = self.module_factories["server"].make_modules(
            self.application, new_modules.get("client", [])
        )
...
class ServerModuleFactory(object):
    ...
    def make_modules(self, application, client_modules):
        ...
        for section in self.server_sections:
            ...
            servers.setdefault((port, interface), []).append(
                (section.name, section.module, section.config)
            )
...
class CloudModuleFactory(object):
    def __init__(self, sections):
        if len(sections) > 1:
            raise config_error.ConfigError(
                "Configuring multiple [cloud] sections isn't supported."
            )
        self.sections = sections
    ...
    def make_modules(self, application):
        ...
        ldap_plugin = drpc_plugins.ldap_directory_sync.LdapSyncDrpcPlugin(
            section.config
        )
...
class SSOModuleFactory(object):
    ...
    def credentials_from_sections(self):
        ...
        for section in self.sections:
            ...
            creds[section.config.get_str("rikey")] = ldap_base.ServiceAccountCredential(
                ...
                password=section.config.get_protected_str(
                    "service_account_password_protected", "service_account_password", ""
                ),
            )
    ...
    def _add_cloudsso_secrets_to_config(self, section_config):
        ...
        section_config.update(
            {
                const.DRPC_PROXY_KEY_IDENTIFIER: secret_storage.retrieve_secret(
                    const.DRPC_PROXY_KEY_IDENTIFIER
                ),
                const.DRPC_API_HOST_IDENTIFIER: secret_storage.retrieve_secret(
                    const.DRPC_API_HOST_IDENTIFIER
                ),
                const.DRPC_SIGNING_SKEY_IDENTIFIER: secret_storage.retrieve_secret(
                    const.DRPC_SIGNING_SKEY_IDENTIFIER
                ),
                const.DRPC_ENCRYPTION_SKEY_IDENTIFIER: secret_storage.retrieve_secret(
                    const.DRPC_ENCRYPTION_SKEY_IDENTIFIER
                ),
            }
        )
