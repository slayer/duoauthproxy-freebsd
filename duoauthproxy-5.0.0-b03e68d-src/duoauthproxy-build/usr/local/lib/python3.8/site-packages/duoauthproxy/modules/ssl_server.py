...
class ChainingOpenSSLContextFactory(ssl.DefaultOpenSSLContextFactory, object):
    ...
    def __init__(
        ...
        if not bool(privatekey_filename) ^ bool(privatekey_data):
            raise ValueError(
                "Please specify one of 'privatekey_filename' or 'privatekey_data'"
            )
        if not bool(certificate_filename) ^ bool(certificate_data):
            raise ValueError(
                "Please specify one of 'certificate_filename' or 'certificate_data'"
            )
        ...
        self.context_factory = (
            context_factory if context_factory is not None else CertificateOptions
        )
    ...
    def cacheContext(self):
        ...
        try:
            certOptions = self.context_factory(
                insecurelyLowerMinimumTo=self.minimum_tls_version,
    ...
    def _select_cipher_list(cipher_list=None):
        ...
        if fips_manager.status():
            return AcceptableCiphers.fromOpenSSLCipherString(
    ...
    def _set_private_key(self, context):
        if self.privatekey_filename:
            try:
                ...
                if (
                    e.args[0][0][1] == "fopen"
                    and e.args[0][2][1] == "SSL_CTX_use_PrivateKey_file"
                ):
                    raise ConfigError(
                        "Could not open private key file at %s"
    ...
    def _set_certificate(self, context):
        if self.certificate_filename:
            try:
                ...
                if (
                    e.args[0][0][1] == "fopen"
                    and e.args[0][2][1] == "SSL_CTX_use_certificate_file"
                ):
                    raise ConfigError(
                        "Could not open certificate file at %s"
