...
def EncodeOctets(str):
    ...
    # BEGIN DUO EDIT @smaxfield
    # This incorrectly assumes that any byte string starting with 0x is a
    # hex value, causing exceptions when bytes stored on an octets attribute
    # happen to begin with 0x. We never store hex in octets attributes, so
    # commenting this out (and effectively reverting to pre-pyrad2.2 behavior)
    # solves the problem.
    #
