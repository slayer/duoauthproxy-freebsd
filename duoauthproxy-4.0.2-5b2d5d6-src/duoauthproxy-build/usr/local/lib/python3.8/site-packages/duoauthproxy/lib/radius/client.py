...
class RadiusClient(protocol.DatagramProtocol):
    def __init__(self, addrs, nas_ip, secret, retries,
                 retry_wait, debug, pw_codec='utf-8'):
        self.addrs = addrs
        if len(self.addrs) < 1:
    ...
    def handle_response(self, datagram, source):
        response_packet = packet.AuthPacket(packet=datagram, dict=base.radius_dictionary())
        ...
        if (source[0], int(source[1])) not in self.addrs:
            raise packet.PacketError(
                'response packet from unknown address: %s:%s'
                % source,
            )
        ...
        try:
            ...
            raise packet.PacketError('unrecognized id in response packet: %s' % response_packet.id)
        ...
        if response_packet.message_authenticator:
            if not response_packet.verify_message_authenticator(original_authenticator=request.packet.authenticator):
                raise packet.PacketError('Invalid Message-Authenticator from {0}'.format(source[0]))
        response_packet.authenticator = request.packet.authenticator

        # clear request state
        self._request_done(request)

        log.msg('Got response for id %r from %r; code %r'
