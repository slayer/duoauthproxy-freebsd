...
def protect(plain_text: str) -> str:
    ...
    cipher_text = win32crypt.CryptProtectData(plain_text.encode(), description, __entropy, reserved_field, prompt_struct, flags)
...
def unprotect(cipher_text: str) -> str:
    ...
    prompt_struct = None
    flags = CRYPTPROTECT_UI_FORBIDDEN | CRYPTPROTECT_LOCAL_MACHINE
    _, plainText = win32crypt.CryptUnprotectData(cipher_text_bytes, __entropy, reserved_field, prompt_struct, flags)
