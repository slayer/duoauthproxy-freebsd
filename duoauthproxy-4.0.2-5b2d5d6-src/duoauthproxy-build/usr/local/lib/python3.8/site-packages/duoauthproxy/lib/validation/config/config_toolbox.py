...
class ConfigTestToolbox(object):
    ...
    def test_config_has_value(self, config, key, optionally_protected=False):
        ...
        if optionally_protected:
            ...
            return config_key_tools.is_key_usable(config, key) or \
                config_key_tools.is_key_usable(config, protected_key)
        else:
    ...
    def test_config_has_key(self, config, key, optionally_protected=False):
        ...
        if optionally_protected:
            ...
            if self.test_key_is_dynamic(config, key):
                ...
                protected_key = "{0}_protected_{1}".format(key_parts.group(1), key_parts.group(2))
            else:
                protected_key = key + '_protected'
            return config_key_tools.is_key_present(config, key) or \
                config_key_tools.is_key_present(config, protected_key)
        else:
    ...
    def test_valid_port(self, config, key):
        ...
        """
        return type_tools.is_intable(config, key) and \
            1 <= int(config.get(key)) <= 65535
    ...
    def test_items_paired(self, config, key1, key2):
        ...
        """
        return (config_key_tools.is_key_present(config, key1) and
                config_key_tools.is_key_present(config, key2))
