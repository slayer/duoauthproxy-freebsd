...
from duoauthproxy.lib import log, const, drpc_keys_rotation, secret_storage
...
class Module(DrpcServerModule):
    ...
    def __init__(self, config):
        ...
        self.duo_client = self.make_duo_client(duo_creds=self.duo_creds,
                                               host=self.host,
    ...
    def perform_join(self):
        try:
            ...
            log.msg('There was a problem with the Duo API credentials.  If this persists, you may need to reconnect your Authentication Proxy.')
            self.stopService()
    ...
    def rotate_and_rejoin(self):
        new_signing_skey, new_encryption_skey = yield drpc_keys_rotation.rotate_skeys(self.duo_client)
        ...
        try:
            ...
            log.err(e, "Failed to rejoin after rotation")
            raise e
    ...
    def restart_looping_call(self, reason):
        if self._check_connection_lc.running:
            ...
            self.reactor.callLater(delay, self._check_connection_lc.start, self.reconnect_interval, True)

    def update_secrets(self, new_signing_skey: bytes, new_encryption_skey: bytes) -> None:
        # Update module's copy
        self.duo_creds = DuoCreds(identity=self.duo_creds.get_identity(), secret=new_signing_skey)

        # Propagate to interested plugins
        self.register_new_parameters({'signing_skey': new_signing_skey, 'encryption_skey': new_encryption_skey})

        # Store in secret storage
        secret_storage.store_secret(const.DRPC_SIGNING_SKEY_IDENTIFIER, new_signing_skey.decode('utf-8'))
        secret_storage.store_secret(const.DRPC_ENCRYPTION_SKEY_IDENTIFIER, new_encryption_skey.decode('utf-8'))

    def log_connect(self, rpc_server):
        if rpc_server and rpc_server.transport.connected:
            log.sso_standard(msg='sso connection established', event_type=log.SSO_EVENT_TYPE_CONNECTIVITY, proxy_key=self.duo_creds.identity)

    def log_disconnect(self, rpc_server):
        if rpc_server and rpc_server.transport.disconnected:
            log.sso_standard(msg='sso connection disconnected', event_type=log.SSO_EVENT_TYPE_CONNECTIVITY, proxy_key=self.duo_creds.identity)
