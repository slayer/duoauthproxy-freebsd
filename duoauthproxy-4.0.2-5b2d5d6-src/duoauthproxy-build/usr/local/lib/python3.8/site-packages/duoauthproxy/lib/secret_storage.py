...
LINUX_STORAGE_PATHS = [os.path.join(os.sep, 'etc', 'duoauthproxy', 'secrets'),
                       os.path.join(util.get_home_dir(), 'conf', '.secrets')]
WINDOWS_STORAGE_PATHS = [os.path.join('C:', os.sep, 'programdata', 'Duo Authentication Proxy', 'secrets'),
                         os.path.join(util.get_home_dir(), 'conf', 'secrets')]
...
def _write_secret(filename: str, identifier: str, secret: str) -> None:
    ...
    with open(filename, 'a+') as fh:
        try:
            ...
            raise SecretStorageDataError("Invalid JSON detected in secret storage. Please clear out storage file and try again.")

        try:
            data[identifier] = value_to_write
            serialized_secrets = json.dumps(data)
        except (TypeError, json.decoder.JSONDecodeError):
            raise SecretStorageDataError("Attempted to write invalid JSON to secret storage. Aborting update.")
        else:
            fh.truncate(0)
...
def restrict_permissions(path: str) -> None:
    ...
    if util.is_windows_os():
        ...
        if os.path.exists(path):
            ...
            win32api.SetFileAttributes(path, existing_attributes | win32con.FILE_ATTRIBUTE_HIDDEN)
    else:
...
def _find_usable_path(storage_paths: List[str]) -> str:
    ...
    raise SecretStorageFileError('No writable storage locations found. Make sure you have the privilege to write.')
