...
class LDIF(basic.LineReceiver, object):
    ...
    def parseValue(self, val):
        if val.startswith(b':'):
            ...
            # Mainline ldaptor is keeping this for python2 backward compatibility in their CI
            # we no longer need to support this and the deprecation warnings are deafening in the
            # test suite so we're switching over
