...
# XXX DUO EDIT
def utf8_binary_to_wire(self):
    """
    D49704: This method will turn a python3 string that contains
    binary data into a 1 byte per character byte string.
    When a python string is created from binary data it is stored internally as
    utf-8. The utf-8 encoding uses an extra byte for each character being stored if the value of
    the byte/character is in the range of 128-255. So, when we need to turn this data back into
    its raw binary we need to use a different encoding that won't incorrectly add in extra bytes.
    For this method's one usage and information about why you would ever use this function
    in real life see the doc string on the LDAPAttributeValueAssertion.toWire() method.
    """
    value = self.value.encode('latin-1')  # latin-1 uses 1 byte for 0-255
    result = (
...
class LDAPBindRequest(LDAPProtocolRequest, BERSequence):
    ...
    def fromBER(klass, tag, content, berdecoder=None):
        ...
        # DUO EDIT D46278: Added SASL support to LDAPBindRequest
        # Change originally made in
        # https://github.com/akgood/ldaptor/commit/f7440e50350710c0355f33d766f2ac10df8f5fd6
        sasl = False
        auth = None
    ...
    def __repr__(self):
        ...
        l.append('auth=%s' % repr(repr_converter(self.auth if getattr(self, 'is_logging_insecure', False) else '****')))
        # END EDIT
        if self.tag != self.__class__.tag:
            l.append('tag=%d' % self.tag)
...
class LDAPResult(LDAPProtocolResponse, BERSequence):
    ...
    def __repr__(self):
        ...
        # DUO EDIT D46278: Added SASL support to LDAPBindRequest
        if self.serverSaslCreds:
            l.append('serverSaslCreds=%r' % self.serverSaslCreds)
        # END EDIT
        return self.__class__.__name__ + '(' + ', '.join(l) + ')'
...
class LDAPBindResponse(LDAPResult):
    ...
    # DUO EDIT D46279: Added __str__ method for backward compatibility with AP code
    # twisted ldaptor has opted to use the `toWire` method rather than the
    # built-in __str__ method for serialization. Since this happened while
    # the authproxy ldaptor was diverged from the mainline, this has been
    # added in for backward compatibilty with old authproxy code
    def __str__(self):
        return self.toWire()
    # END EDIT
...
class LDAPAttributeValueAssertion(BERSequence):
    ...
    def toWire(self):
        # XXX BEGIN DUO EDIT
        # XXX D49704: Ldaptor turns ever single field of an ldapfilter that it encounters into a utf-8 string.
        # XXX While this is correct for the majority of use cases if your filter text contains a field
        # XXX with binary data a utf-8 encoding is almost certainly not the right choice.
        # XXX See utf8_binary_to_wire() for the reason why this encoding messes with the data.
        # XXX Due to the way the parsefilter logic works there is currently no simple way to
        # XXX detect when a field should not be decoded as utf-8 and instead treated specially.
        # XXX This forces us to use this nasty hack where we check for a certain attribute and if
        # XXX it's in our whitelist of binary attributes we will overwrite the toWire() method to use
        # XXX an encoder that will return the data to its intended binary form.

        if self.attributeDesc.value.lower() in BINARY_ATTRIBUTES:
            self.assertionValue.toWire = functools.partial(utf8_binary_to_wire, self.assertionValue)
        # XXX END DUO EDIT
...
class LDAPFilter_substrings(BERSequence):
    ...
    def __repr__(self):
        ...
        if self.tag==self.__class__.tag:
            return self.__class__.__name__\
                   +"(type=%s, substrings=%s)"\
                   %(repr(tp), repr(self.substrings))
        else:
            return self.__class__.__name__\
                   +"(type=%s, substrings=%s, tag=%d)"\
                   %(repr(tp), repr(self.substrings), self.tag)
            ...
            return self.__class__.__name__ \
                   + ("(baseObject=%s, scope=%s, derefAliases=%s, " \
                     ...
                     % (repr(base), self.scope,
                        self.derefAliases, self.sizeLimit,
                        self.timeLimit, self.typesOnly,
                        repr(self.filter), self.attributes, self.tag)
...
class LDAPControl(BERSequence):
    ...
    def fromBER(klass, tag, content, berdecoder=None):
        ...
        # Review of this change https://phab.duosec.org/T53931
        kw = {}
        index = 1
        if l[index:] and isinstance(l[index], BERBoolean):
            kw['criticality'] = l[index].value
            index += 1
        else:
            kw['criticality'] = None

        if l[index:] and isinstance(l[index], BEROctetString):
            kw['controlValue'] = l[index].value
            index += 1
        else:
            kw['controlValue'] = None
    ...
    def __repr__(self):
        ...
        if self.tag==self.__class__.tag:
            ...
            return self.__class__.__name__+"(entry=%s, attributes=%s, tag=%d)" \
                   %(repr(entry), repr(self.attributes), self.tag)
