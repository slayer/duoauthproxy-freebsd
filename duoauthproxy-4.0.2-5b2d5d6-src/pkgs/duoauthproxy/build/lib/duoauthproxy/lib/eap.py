...
class EAPSession(object):
    def __init__(self,
                 ...
                 cipher_list: str = '',
                 minimum_tls_version: Optional[str] = None) -> None:
        ...
        self.current_request: packet.AuthPacket
    ...
    def start_tls(self, pkey_file, certs_file, cipher_list, minimum_tls_version):
        ...
        self.innerEAP = EAPSession(inner=True, gtc_message=self.gtc_message, parent=self)
    ...
    def add_incoming(self, msg: bytes):
        ...
        try:
            ...
            header = chr(2).encode() + msg[1:2] + struct.pack('>H', len(decrypted_msg) + 4)  # bytes
            ...
            log.auth_standard(msg=ssl_error,
    ...
    def next_message(self) -> Iterator[bytes]:
        ...
        if self.state == EAP_SESSION_START:
            # Request Identity
            packet = EAPPacket(EAP_CODE_REQUEST, self.id, EAP_TYPE_IDENTITY, chr(0).encode())
            ...
            self.start_tls(self.pkey, self.certs, self.cipher_list, self.minimum_tls_version)
            packet = EAPPacket(EAP_CODE_REQUEST, self.id, EAP_TYPE_PEAP, b'', start=True)
            ...
            if outgoing_msg:
                packet = EAPPacket(EAP_CODE_REQUEST, self.id, EAP_TYPE_PEAP, outgoing_msg)
                ...
                if isinstance(inner_msg, bytes):
                    ...
                    log.auth_standard(msg=repr(inner_msg),
                                      username=self.current_request.username,
                                      auth_stage=log.AUTH_UNKNOWN,
                                      status=log.AUTH_ERROR,
                                      client_ip=self.current_request.client_ip,
                    ...
                    log.auth_standard(msg=peap_error_message,
                                      username=self.current_request.username,
                                      auth_stage=log.AUTH_UNKNOWN,
                                      status=log.AUTH_ERROR,
                                      client_ip=self.current_request.client_ip,
            ...
            packet = EAPPacket(EAP_CODE_REQUEST, self.id, EAP_TYPE_GTC, self.gtc_message)
        ...
        if self.inner:
            defer.returnValue(packet.render()[4:])  # tunneled EAP skips code, id, length
    ...
    def add_message(self, msg: bytes):
        ...
        if not self.inner and self.id != 0 and identifier != self.id:
            ...
            log.msg("Response ID %s does not match request ID %s" % (identifier, self.id))
            defer.returnValue(False)
        if len(msg) != length:
            message = 'Malformed EAP packet: length ({0}) does not match actual length ({1})'.format(length, len(msg))
            log.auth_standard(msg=message,
                              username=self.current_request.username,
                              auth_stage=log.AUTH_UNKNOWN,
                              status=log.AUTH_ERROR,
                              client_ip=self.current_request.client_ip,
