...
from twisted.logger import PredicateResult, FileLogObserver, formatEvent
...
class BufferingLogger(object):
    ...
    def msg(self, *args, **kwargs):
        if not self._ready:
            self._calls.append((self._log.msg, args, kwargs))
        else:
    ...
    def config(self, config, scrub=lambda x: False):
        ...
        if not self._ready:
            self._calls.append((self._log.msg, [result], {}))
        else:
            self._log.msg(result)

    def err(self, *args, **kwargs):
        if not self._ready:
            self._calls.append((self._log.err, args, kwargs))
        else:
            self._log.err(*args, **kwargs)

    def auth(self, **kwargs):
        ts = datetime.utcnow().isoformat() + 'Z'
        arguments = {'timestamp': ts}
        arguments.update(kwargs)
        msg_type = MSG_TYPE_AUTH
        if not self._ready:
            self._calls.append((self._log.msg, [''], {MSG_TYPE_KEY: msg_type, AUTH_VALUES_KEY: arguments}))
        else:
            self._log.msg('', msg_type=msg_type, auth_values=arguments)

    def auth_standard(self, msg, username, auth_stage, status, client_ip, server_section, factor=None, server_section_ikey=None):
    ...
    def sso(self, **kwargs):
        ...
        if not self._ready:
            self._calls.append((self._log.msg, [''], {MSG_TYPE_KEY: msg_type, SSO_VALUES_KEY: arguments}))
        else:
            self._log.msg('', msg_type=msg_type, sso_values=arguments)
    ...
    def sso_ldap(self, msg, query_type, status, server, port, username, proxy_key, reason=None):
...
def auth_type_predicate(event):
    """
    Implementor of the predicate interface used in conjunction with the FilteringLogObserver in proxy.py.

    This should detect events that should be directed to the auth log file.  Make sure the criteria here and in the
    log.auth method above match.
    """
    if event.get(MSG_TYPE_KEY) == MSG_TYPE_AUTH:
...
def sso_type_predicate(event):
    if event.get(MSG_TYPE_KEY) == MSG_TYPE_SSO:
        return PredicateResult.yes
...
def only_default_log_predicate(event):
    if event.get(MSG_TYPE_KEY) != MSG_TYPE_SSO and event.get(MSG_TYPE_KEY) != MSG_TYPE_AUTH:
        return PredicateResult.yes
...
def format_auth_event(event):
    """
    Event formatter used to prepare a string suitable to go to the auth log file.

    The event will have properties based on the keyword arguments generated by the log.auth method above

    :param event: a twisted log event generated by the logging framework
    :return: a unicode string suitable for inclusion in the auth log file
    """
    auth_values = event.get(AUTH_VALUES_KEY)
    return json.dumps(auth_values) + '\n'


def format_sso_event(event):
    """
    :param event: a twisted log event generated by the logging framework
    :return: a unicode string suitable for inclusion in the auth log file
    """
    sso_values = event.get(SSO_VALUES_KEY)
    return json.dumps(sso_values) + '\n'
