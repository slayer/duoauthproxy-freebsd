...
STARTTLS_NOT_SUPPORTED_ERROR_MESSAGE = "Received starttls request for ldap_server_auto without proper tls " \
...
class Proxy(ldapserver.BaseLDAPServer, object):
    ...
    def connectionMade(self):
        ...
        try:
            ...
            self.__client.unsolicited_notification_handler = self._handle_unsolicited_notifications
            self.__client.upstream_server_disconnect_handler = self._handle_upstream_server_disconnect
            ...
            if self.debug and self.transport.connected:
                try:
                    ...
                    log.msg("Connection made between client: {client_host}:{client_port} and the server section listening via {server_host}:{server_port}.".format(
    ...
    def connectionLost(self, reason):
        ...
        if reason.check(SSL.Error):
            ...
            log.msg("The downstream application and the Authentication Proxy were not able to establish an SSL connection. It is possible this is because of a TLS protocol mismatch."
                    ...
                    " if you need to use a lower version of the TLS Protocol. Error message: {}.".format(self.server_section_name, reason.getErrorMessage()))
        elif self.debug:
            log.msg("Closing the connection between the downstream application and the Authentication Proxy. Reason: {}".format(reason.getErrorMessage()))
    ...
    def _handle_unsolicited_notifications(self, msg):
        ...
        if msg.resultCode == ldaperrors.LDAPUnavailable.resultCode:
            log.msg("Dropping connection between proxy and client due to upstream server unavailability")
            self._tear_down_connection()
    ...
    def _handle_upstream_server_disconnect(self, reason, host, port):
        ...
        if self.connected:
            log.msg("The upstream LDAP server ({host}:{port}) has closed its connection to the proxy. Now closing our connection to the downstream client. Reason given for close: {reason}.".format(host=host, port=port, reason=reason))
            self._tear_down_connection()
    ...
    def handle_LDAPExtendedRequest(self,
                                   request: LDAPExtendedRequest,
                                   controls: pureldap.LDAPControl,
                                   reply_fn: Callable[..., None]) -> Any:
        ...
        try:
            ...
            self.transport.startTLS(self.sslctx,
                                    self.factory)
    ...
    def _cbHandle(self, *args, **kwargs):
        try:
            return ldapserver.BaseLDAPServer._cbHandle(self,
                                                       *args,
                                                       **kwargs)
...
if __name__ == '__main__':
    ...
    config = ldaptor.config.LDAPConfig(serviceLocationOverrides={
        '': (backing_host, backing_port),
    })
    factory = twisted.internet.protocol.ServerFactory()
    factory.protocol = functools.partial(ProxyToLDAPConfig, config)
    reactor.listenTCP(listening_port,
                      factory)
    reactor.run()
