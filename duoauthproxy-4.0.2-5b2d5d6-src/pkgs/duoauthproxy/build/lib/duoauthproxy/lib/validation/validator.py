...
from duoauthproxy.lib.validation.test_resolver import STANDARD_CONFIG_RESOLVER, STANDARD_CONNECTIVITY_RESOLVER

from duoauthproxy.lib.log import _sanitize_config


NO_SECTIONS_MESSAGE = "The Auth Proxy configuration has no sections - testing will be skipped."
...
@defer.inlineCallbacks
def check_config(full_configuration, logger, toolbox=STANDARD_CONFIG_TOOLBOX, resolver=STANDARD_CONFIG_RESOLVER):
    validator = ConfigAndConnectivityValidator(config_toolbox=toolbox,
...
@defer.inlineCallbacks
def check_connectivity(full_configuration, logger, toolbox=STANDARD_TOOLBOX, resolver=STANDARD_CONNECTIVITY_RESOLVER):
    validator = ConfigAndConnectivityValidator(connectivity_toolbox=toolbox,
...
class ConfigAndConnectivityValidator(object):
    ...
    def __init__(self, config_toolbox=STANDARD_CONFIG_TOOLBOX,
        ...
        try:
            ...
            if self.validate_connectivity:
                ...
                if self.validate_config and not all_sections_successful(config_results):
                    ...
                    connectivity_results = yield self._validate_connectivity(full_configuration, logger)
            ...
            logger.error("There was a problem running the connectivity tool: {error}", error=error)
            util.set_stdout_color('reset')
            defer.returnValue(({}, {}))

    @defer.inlineCallbacks
    def _validate_config(self, full_configuration, logger):
        config_results = yield self._validate(full_configuration, logger,
                                              self.config_toolbox,
                                              self.config_test_resolver)
        cross_config_result = self.cross_config_tester(full_configuration)
        config_results.update({
            self.CROSS_CONFIG_KEY: cross_config_result
        })
        defer.returnValue(config_results)

    @defer.inlineCallbacks
    def _validate_connectivity(self, full_configuration, logger):
        connectivity_results = yield self._validate(full_configuration, logger,
    ...
    def _validate(self, full_configuration, logger, toolbox, section_test_resolver):
        ...
        all_results = yield self._test_all_sections(sections, toolbox, section_test_resolver)
    ...
    def _test_all_sections(self, sections, toolbox, section_test_resolver):
        ...
        for section_name, section_config in sections.items():
            ...
            section_result = yield self._test_one_section(section_config, toolbox, section_tester)
    ...
    def _test_one_section(self, section_config, toolbox, section_tester):
        ...
        """
        result = yield defer.maybeDeferred(section_tester, section_config, toolbox=toolbox)
    ...
    def _output_results(self, all_results, sections, logger):
        ...
        for section_name, section_result in all_results.items():
            logger.info("Testing section \'{section_name}\' with configuration:", section_name=section_name)
            logger.info("{cfg}", cfg=_sanitize_config(
                sections[section_name], lambda x: x.startswith('radius_secret') or
                x.startswith('secret') or
    ...
    def _log_summary_section(self, config_results, connectivity_results,
        ...
        """
        logger.info("SUMMARY")
        successful = all_sections_successful(config_results) and all_sections_successful(connectivity_results)
        has_warnings = any_section_is_warning(config_results) or any_section_is_warning(connectivity_results)
    ...
    def _log_summary_for_results(self, config_results, connectivity_results, logger):
        ...
        if cross_config_result and self._has_summary_to_print(cross_config_result):
            self._log_section_summary([cross_config_result], logger, filtering_predicate, "Cross-section results")

        # Then log the summaries for the real config sections
        self._log_section_summaries(config_results, connectivity_results, logger, filtering_predicate)
        # Put the logger's observer back to its original value
        logger.observer = original_observer

    def _log_section_summaries(self, config_results, connectivity_results, logger, filtering_predicate):
        """
        ...
        for section in tested_sections:
            ...
            connectivity_result = connectivity_results.get(section, SkippedSectionResult())
            if self._has_summary_to_print(config_result) \
                ...
                self._log_section_summary(section_results, logger, filtering_predicate, header)
