        ...
        with open(ssl_cert_path) as fd:
            ...
            if _contains_truncated_certs(cert_input):
                raise Exception(
                    ...
                    "'-----BEGIN CERTIFICATE-----' and end with '-----END CERTIFICATE-----'.".format(ssl_cert_path)
                )
...
def _can_create_ssl_context(ssl_key, ssl_certs, cipher_list, minimum_tls_version=None):
    ...
    if not ssl_key or not ssl_certs:
        return UnmetPrerequisiteSkippedTestResult('SSL context creation', 'SSL key and cert')
...
def can_validate_ssl_creds(ssl_key_path, ssl_cert_path, cipher_list=None, minimum_tls_version=None):
