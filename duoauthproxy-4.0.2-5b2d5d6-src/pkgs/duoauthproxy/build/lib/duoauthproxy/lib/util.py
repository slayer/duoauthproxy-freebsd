...
def parse_ldap_filter(filter_string):
    ...
    try:
        ...
        log.msg("Invalid LDAP Filter: {0}. Exception: {1}".format(filter_string, str(e)))
        return None
...
def factor_for_request(preferred_factors, preauth_res):
    ...
    user_factors = set(
        tokenize(factor)
        for factor in preauth_res['factors'].values()
    )
...
def _set_home_dir():
    ...
    if hasattr(sys, "frozen") and sys.frozen in ("windows_exe", "console_exe"):
        ...
        _home_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), *['..'] * 7))
...
def get_host_list(config):
    ...
    """
    base_key = 'host'
    hosts = [config.get_str(host_key) for host_key in get_dynamic_keys(config, base_key)]
    if not hosts:
...
def get_ssl_ca_certs(config):
    ...
    if ssl_ca_certs_file:
        try:
            ...
            raise ConfigError(
                'Could not load ssl ca certificates from \'%s\''
                % ssl_ca_certs_file)
...
def warn_insecure_settings(auth_type, transport_type):
    ...
    Args:
        auth_type (str): One of const.AD_AUTH_TYPES
        transport_type (str): One of const.AD_TRANSPORTS
    Returns:
        str: Warning message
    """
    msg = ''
    if (auth_type == const.AD_AUTH_TYPE_PLAIN and
            transport_type == const.AD_TRANSPORT_CLEAR):
    ...
    if (auth_type == const.AD_AUTH_TYPE_NTLM_V1 and
            transport_type == const.AD_TRANSPORT_CLEAR):
...
def parse_ad_client(config):
    ...
    auth_type = config.get_enum(
        'auth_type', const.AD_AUTH_TYPES,
        const.AD_AUTH_TYPE_NTLM_V2, str.lower)
    ...
    transport_type = config.get_enum(
        'transport', const.AD_TRANSPORTS,
        const.AD_TRANSPORT_CLEAR, str.lower)
    ntlm_domain = config.get_str('ntlm_domain', '')
    domain_discovery = config.get_bool('domain_discovery', False)
    ntlm_workstation = config.get_str('ntlm_workstation', '')
    ssl_verify_depth = config.get_int('ssl_verify_depth',
                                      const.DEFAULT_SSL_VERIFY_DEPTH)
    ...
    if (transport_type in (const.AD_TRANSPORT_LDAPS, const.AD_TRANSPORT_STARTTLS)
            and not ssl_verify_hostname and not ssl_ca_certs):
...
def extract_numbers(strings, prefix):
    ...
    """
    return set([re.sub(prefix, '', x) for x in strings if re.match(r'^\d+$', re.sub(prefix, '', x))])
...
def should_try_splitting_password(password: str, allow_concat: bool, delimiter: str, delimited_password_length: int) -> bool:
    if not allow_concat:
        return False
