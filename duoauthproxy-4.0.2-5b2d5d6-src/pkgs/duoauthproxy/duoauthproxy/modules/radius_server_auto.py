...
from ..lib import base, log
from ..lib import duo_async
...
class DuoAutoRadiusServer(ChallengeResponseRadiusServer, DuoSimpleRadiusServer):
        ...
        if 'EAP-Message' in request.packet:
            ...
            if preauth_res['result'] == duo_async.API_RESULT_ALLOW:
                ...
                if primary_res.success:
                    response_packet = self.create_accept_packet(request, radius_attrs=primary_res.radius_attrs)
                else:
                    response_packet = self.create_reject_packet(request, None, primary_res.radius_attrs)
            elif preauth_res['result'] in (duo_async.API_RESULT_DENY,
                ...
                if factor == "passcode":
                    ...
                    self.log_request(request,
                                     'Sending authentication challenge packet')
                    ...
                    if auth_resp['result'] == duo_async.API_RESULT_ALLOW:
                        ...
                        if primary_res.success:
                            response_packet = self.create_accept_packet(
                                request,
                                radius_attrs=primary_res.radius_attrs
                            )
                        else:
                        ...
                        response_packet = self.create_reject_packet(request, msg, radius_attrs=radius_attrs)
            ...
            if primary_res.success:
                ...
                response_packet = yield self.duo_auth(request,
                                                      primary_res,
                                                      factor=None,
                ...
                if self.allow_concat:
                    log.msg("Allow concat is configured, but is not " +
                ...
                if 'MS-CHAP-Error' not in primary_res.radius_attrs:
                    primary_res.radius_attrs['MS-CHAP-Error'] = self.MS_AUTH_FAILED_ERROR
                else:
                    m = self.MS_ERRCODE_RE.match(primary_res.radius_attrs['MS-CHAP-Error'][0])
                    if (m is not None and
                            int(m.group('E')) == self.ERROR_PASSWORD_EXPIRED):
                        log.msg("{0} Rejected due to password expiration".format(request.username))
                        preauth_res = yield self.duo_preauth(request)
                        if preauth_res['result'] in (duo_async.API_RESULT_DENY,
                                                     duo_async.API_RESULT_ENROLL):
            ...
            if util.should_try_splitting_password(password,
                ...
                password_part, factor = util.do_password_split(password, self.delim, self.delimited_password_length)
                if not factor:
                    factor = None
                primary_res = yield self.primary_auth(request,
                                                      password_part)
            else:
                primary_res = None
                factor = None
    ...
    @defer.inlineCallbacks
    def radius_proxy_challenge(self, request):
        # Send request to the RADIUS server and get the result.
        try:
            response_packet = yield self.primary_ator.radius_proxy(request)
        except NotImplementedError:
            self.log_request(request,
                             base.RADIUS_PROXY_NOT_IMPLEMENTED_LOG_MSG)
            ...
            msg = ('Error performing primary authentication: %s'
                   % e)
            self.log_request(request, msg)
            log.auth_standard(msg=msg,
                              username=request.username,
        ...
        primary_res = AuthResult.from_radius_packet(response_packet)
        if response_packet.code == packet.AccessReject:
            res_msg = 'Primary credentials rejected - {0}'.format(primary_res.msg)
            log.auth_standard(msg=res_msg,
                              username=request.username,
                              auth_stage=log.AUTH_PRIMARY,
                              status=log.AUTH_REJECT,
                              client_ip=request.client_ip,
        ...
        elif response_packet.code == packet.AccessAccept:
            log.auth_standard(msg='Primary authentication successful - {0}'.format(primary_res.msg),
                              ...
                              server_section_ikey=self.server_section_ikey)
            primary_res = AuthResult.from_radius_packet(response_packet)
            primary_res.radius_attrs.update(dict(response_packet))
            response_packet = yield self.duo_auth(request,
                                                  primary_res)
        elif (response_packet.code == packet.AccessChallenge
              and 'State' in response_packet):
    ...
    @defer.inlineCallbacks
    def duo_auth(self, request, primary_res,
                 factor=None,
                 preauth_res=None,
    ...
    @defer.inlineCallbacks
    def get_passcode_challenge_response(self, request, state, radius_reject_attrs=None):
        ...
        if passcode:
            ...
            self.log_request(request,
                             base.NOT_PAP_PASSWORD_CLIENT_MSG)
            msg = 'Missing or improperly-formatted password'
            log.auth_standard(msg=msg,
                              username=request.username,
                              auth_stage=log.AUTH_SECONDARY,
                              ...
                              server_section=self.server_section_name,
                              server_section_ikey=self.server_section_ikey)
            response_packet = self.create_reject_packet(request, msg, radius_attrs=radius_reject_attrs)
        defer.returnValue(response_packet)
