...
def get_basic_radius_server_resolver(config, toolbox):
    ...
    add_dynamic_keys_to_test_resolver(config, config_test_resolver, dynamic_test_resolver)

    return config_test_resolver


def add_dynamic_keys_to_test_resolver(config, config_test_resolver, dynamic_test_resolver):
...
def check_common_required_radius_keys(config, toolbox):
    ...
    if not toolbox.test_config_has_any_dynamic_key(config, 'radius_secret', optionally_protected=True):
...
def check_radius_secret_radius_ip_balance(config):
    ...
    radius_ip_numbers, radius_secret_numbers, radius_secret_protected_numbers = get_radius_ip_secret_numbers(config)

    for overlapped_key in radius_secret_numbers.intersection(radius_secret_protected_numbers):
    ...
    for radius_ip in radius_ip_numbers.difference(radius_secret_numbers):
        problems.append(UnpairedKey(key1='radius_secret_' + radius_ip, key2='radius_ip_' + radius_ip))

    for radius_secret in radius_secret_numbers.difference(radius_ip_numbers):
        problems.append(UnpairedKey(key1='radius_ip_' + radius_secret, key2='radius_secret_' + radius_secret))
...
def get_radius_ip_secret_numbers(config):
    ...
    radius_secret_protected_numbers = util.extract_numbers(keys, 'radius_secret_protected_')
...
def check_basic_radius_server_protected_usage(config, toolbox):
    ...
    possible_protected_keys.extend(util.get_all_numbered_keys(config, 'radius_secret_protected'))
